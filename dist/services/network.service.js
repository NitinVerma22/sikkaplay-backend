        ...
        await db_1.prisma.$transaction(async (tx) => {
            for (const [referrerId, totalAmount] of accumulatedCommissions.entries()) {
                ...
                if (referrer?.fcmToken) {
                    (0, push_service_1.sendPushNotification)(referrer.fcmToken, 'You earned Network Income! ⚡', `You earned ${totalAmount} coins from your referral network in the last 3 hours!`, 'alert', null, referrerId).catch(err => console.error(`Error notifying referrer ${referrerId}:`, err));
                }
            }
