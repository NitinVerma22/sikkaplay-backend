...
const awardReferralBalance = async (userId: string, amount: number, description: string) => {
  await prisma.$transaction(async (tx) => {
    ...
    if (userForPush?.fcmToken) {
      sendPushNotification(userForPush.fcmToken, 'You earned Network Income!', `You just received ${amount} coins from your referral network!`, 'alert', null, userId);
    }
  });
};

export const distributePendingReferralCommissions = async (): Promise<void> => {
  ...
  try {
    ...
    const userCacheByReferralCode = new Map<string, any>();
    const getUserByReferral = async (referralCode: string) => {
    ...
    const accumulatedCommissions = new Map<string, number>();
    ...
    for (const tx of pendingTransactions) {
      ...
      if (l1Reward > 0) {
        accumulatedCommissions.set(
          l1Referrer.id,
          (accumulatedCommissions.get(l1Referrer.id) || 0) + l1Reward
        );
        ...
        if (l1Referrer.referredBy) {
          ...
          if (l2Referrer) {
            ...
            if (l2Reward > 0) {
              accumulatedCommissions.set(
                l2Referrer.id,
                (accumulatedCommissions.get(l2Referrer.id) || 0) + l2Reward
              );
              ...
              if (l2Referrer.referredBy) {
                ...
                if (l3Referrer) {
                  ...
                  if (l3Reward > 0) {
                    accumulatedCommissions.set(
                      l3Referrer.id,
                      (accumulatedCommissions.get(l3Referrer.id) || 0) + l3Reward
                    );
                  }
                }
              }
